...
class CQHttp:
    ...
    def get_group_notice(self, group_id: int, **kw) -> dict:
        cookies = dict(Cookie=self.get_cookies()['cookies'])
        csrf_token = self.get_csrf_token()['token']
        url = 'http://web.qun.qq.com/cgi-bin/announce/get_t_list'
        data = dict(bkn=csrf_token, qid=group_id, ft=23, s=-1, n=10, ni=1, i=1)
        data.update(**kw)
        return requests.post(url, data, headers=cookies).json()

    def send_group_notice(self, group_id: int, title: str, text: str, **kw):
        cookies = dict(Cookie=self.get_cookies()['cookies'])
        csrf_token = self.get_csrf_token()['token']
        url = 'http://web.qun.qq.com/cgi-bin/announce/add_qun_notice'
        data = dict(bkn=csrf_token, qid=group_id, title=title, text=text)
